...
# 	data=droplevels(subset(means2, (M!=0 & roi2%in%c("DET/N","RC-V","V")  & variable%in%c("Gaze")))))
...
# 	# + facet_grid(variable~.)
# )
# ggsave("gg05-model+data-rt.pdf", p.d1, width=6, height=5)


dodge1 <- position_dodge(width=.9)
dodge2 <- position_dodge(width=0.07)
dodge3 <- position_dodge(width=0.5)
(p.d1 <- ggplot(aes(factor(roi2,levels=c("V","RC-V")), M, fill=factor(cond, levels=c("SRC","ORC"))),
	data=droplevels(subset(means2, (roi2%in%c("RC-V","V")  & variable%in%c("retr")))))
	+ ylab("Mean attachment duration") 
	+ xlab("Region")
	+ ggtitle("Attachment")
	+ theme_bw()
	+ theme(legend.position="right"
	...
	# + scale_x_discrete(expand=c(0.1,0))
	+ scale_y_continuous(expand=c(0.1,0), limits=c(0,200))
...
ggsave("gg05-model-attachment.pdf", p.d1, width=6, height=5)

dodge1 <- position_dodge(width=.9)
dodge2 <- position_dodge(width=0.07)
dodge3 <- position_dodge(width=0.5)
(p.d1 <- ggplot(aes(factor(roi2,levels=c("V","RC-V")), M, fill=factor(cond, levels=c("SRC","ORC"))),
	data=droplevels(subset(means2, (roi2%in%c("RC-V","V")  & variable%in%c("Gaze")))))
	+ ylab("Mean gaze duration") 
	+ xlab("Region")
	...
	+ scale_color_manual("cond", values=UPpalette(firstcolor="humfak",secondcolor="darkblue"))
	# + scale_x_discrete(expand=c(0.1,0))
	+ scale_y_continuous(expand=c(0.1,0), limits=c(0,500))
	# + geom_line(colour="gray90", linetype=2, aes(y=data))
	+ geom_bar(position=dodge1, stat="identity")
	# + geom_bar(aes(y=data), width=0.1, position=dodge1, stat="identity")
	# + geom_point(position=dodge2)
...
ggsave("gg05-model.pdf", p.d1, width=6, height=5)
