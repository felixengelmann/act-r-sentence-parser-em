...
;;; Felix Engelmann
;;;             : * Made several changes concerning the interaction with parsing module
;;;             : * Changed encoding equation to include surprisal
;;;             : * Added word information output
;;;             : * Added parameters for fixed preparation time
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;
      (unless (eq 'BUSY (exec-s eye-mod))  
        (awhen (prep-event eye-mod)
          (model-warning " +++ Deleting a prep event (SKIP) +++")
               (delete-event it))
...
(defmethod initiate-eye-move ((eye-mod emma-vis-mod) (recog-time number)
  ...
  (let ((sacc-mvmt (build-sacc-mvmt eye-mod r-theta)))
    ...
    (command-output "     +++ Prep-Time: ~6,3F +++ Exec-Time: ~6,3F +++" (fprep-time sacc-mvmt) (exec-time sacc-mvmt))
    (setf (prep-event eye-mod)  ;;mjs
          (schedule-event-relative (max 0 (fprep-time sacc-mvmt))  'PREPARATION-COMPLETE
                                   :destination :vision
                                   :module :vision
                                   :output 'low ;[FE, default: medium]
...
;  (report-fixation-time (write-to-string trgt))
  ...
  (when (and (moving-attention eye-mod)
    ...
    (let* ((start-loc (eye-loc eye-mod))
      ...
      (command-output "*** ~6,3F   Refixation! New encoding time: ~F +++" (mp-time) new-duration)
...
(defmethod perform-movement ((eye-mod emma-vis-mod) (sacc saccade))
  (if (eq (exec-s eye-mod) 'BUSY)   ;[FE]
    (print-warning "Eye move could not execute at ~S while one was already in progress."
                (mp-time))
    (progn
...
(defun report-attention-shift (obj freq enctime eccentricity srpr)
 (no-output (when (car (sgp :v)) 
  (let ((word (chunk-slot-value-fct obj 'value)))
    (command-output "*** ~6,3F   Encoding ~D" (mp-time) word)
    (command-output "     +++ Enctime: ~3,3F +++ Freq.: ~3,5F +++ Ecc.: ~3,5F +++ Surpr.: ~3,5F +++" 
                    enctime freq eccentricity srpr)
))))

;; [FE] dummy functions
;; TODO: make dummy functions more useful
(defun output-enctime (obj enctime) obj enctime nil)
(defun my-frequency (obj) obj nil)
(defun report-fixation-time (loc) loc nil)
(defun surprisal (obj) obj 0)
